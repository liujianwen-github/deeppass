...
//var vahost = "http://101.201.106.53";// 正式
...
var config = {
	...
	// cardInfoURL : vahost + '/deepFace/readCardInfo',
	...
	matchscore : 0.5,// 匹配值
	addFaceTrackPercent : 0.9,// 若vip的匹配度小于阈值，添加到vip中
	vipValue : 1,// vip
	runTime : 0.5 * 1000,// 访问接口的时间间隔
	notMatchStatus : false,// 未识别标志，未true时不会再拿数据
	clearFaceTrackids : 10 * 1000,// 清空本地faceTrack池时间间隔
	runCount : 0,// 访问次数：大于clearFaceTrackids/runTime时清空facetrack池
};
/**
 * TODO 门状态
 */
		...
		if (objDetail.count > 0) {
			...
			if (results != "") {
				if (results.Vip == config.vipValue
					...
					if (objDetail.matchs[0].percent < config.addFaceTrackPercent) {
						addFaceTrackToPerson(results.personId, objDetail.id);// 小于vip阈值添加
					}
					// 添加faceTrack
				} else {
