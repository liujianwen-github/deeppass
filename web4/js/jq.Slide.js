(function($) {
	$.fn.Slide = function(options) {
		...
		// 滚动对象的子元素宽度，相当于滚动的宽度
		var slideW = ContentBox.parent().width();// 滚动对象的子元素宽度，相当于滚动的宽度
		slideW = slideW / 2;
		var autoPlay;
		var slideWH;
		...
		return this.each(function() {
			...
			// 滚动函数
			var doPlay = function() {
				$.fn.Slide.effect[opts.effect](ContentBox, targetLi, index,
						slideWH, opts);
				index++;
			...
			clickNext.click(function(event) {
				...
				$.fn.Slide.effectLoop.scroolLeft(ContentBox, targetLi, index,
						slideWH, opts, function() {
							for (var i = 0; i < opts.steps; i++) {
								ContentBox.find("li:first", $this).appendTo(
										ContentBox);
							}
							ContentBox.css({
								"left" : "0"
			...
			clickPrev.click(function(event) {
				...
				$.fn.Slide.effectLoop.scroolRight(ContentBox, targetLi, index,
						slideWH, opts);
				event.preventDefault();
			});
			// 自动播放
