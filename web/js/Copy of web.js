	...
	if (json.code == json.SUCC_CODE && json.knowList != null) {
		...
		for (var i = 0; i < count; i++) {
			...
			var str = 'yname="' + json.name + '" ytime="'
					...
					+ json.matchId + '" imgUrl="data:image/jpg;base64, '
					+ json.imageId + '" registTime="'
...
function preCreateUser(imageUrl, matchId) {
	...
	str = '<a class="close-reveal-modal">&#215;</a>'
			...
			+ imageUrl
			+ '" alt="头像" /> <a'
			+ 'href="javascript:void(0);" title="修改头像"></a>'
			+ '</div>'
			+ '<div class="addUser">'
			...
			+ 'value="0" class="radio" ></div>'
			+ '</div>'
			+ '</div>'
			+ '</div>'
			+ '<div class="dialogBottom">'
			+ '<a href="javascript:void(0);" title="取消" class="close-reveal-modal" data-reveal-id="intelligentAnalysis" >取消</a>'
			+ '<a href="javascript:void(0);" id="Imsure" title="确定" class="close-reveal-modal close-reveal-modal-sure"'
